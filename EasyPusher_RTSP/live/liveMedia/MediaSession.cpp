  ...
  while (sdpLine != NULL) {
    ...
    if (subsession == NULL) {
      ...
      return False;
		...
		subsession->setTrackIndex(index);index++;
...
char* MediaSession::lookupPayloadFormat(unsigned char rtpPayloadType,
  ...
  case 97: {temp = "METADATA"; freq = 90000; nCh = 1; break;}	
  };
...
Boolean MediaSubsession::createSourceObjects(int useSpecialRTPoffset) {
  do {
	...
	// this sets "durationInMicroseconds" correctly, based on the PCR values
	  ...
	  if(LIVEMEDIA_EASYDARWIN_RELAY_MARK == useSpecialRTPoffset)
	  {
	...
	delete[] mimeType;
		   ...
		   || strcmp(fCodecName, "METADATA") == 0 // METADATA
		   ) {
	createSimpleRTPSource = True;
