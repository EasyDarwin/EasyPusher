...
// 		if (!m_pTSDemuxer)
// 		{
// 			m_pTSDemuxer = new ts::demuxer();
// 		}
// 		m_pTSDemuxer->parse_only=dump>0?true:parse_only;
// 		m_pTSDemuxer->es_parse=m_pTSDemuxer->parse_only;
// 		m_pTSDemuxer->dump=dump;
// 		m_pTSDemuxer->av_only=av_only;
// 		m_pTSDemuxer->channel=channel;
// 		m_pTSDemuxer->dst=output;
// 		m_pTSDemuxer->verb=verb;
// 		m_pTSDemuxer->SetCaptureFileCallback(m_pCallback, m_pMaster);

		//for a test
		m_nVideoTrackId = 0;
		m_nAudioTrackId = 1;
		m_mediaInfo.nFps = 25;// frame per second
		m_mediaInfo.nAudioSampleRate = 44100;
		m_mediaInfo.nChannels = 2;
