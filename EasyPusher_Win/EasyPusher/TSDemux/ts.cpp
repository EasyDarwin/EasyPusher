...
//解析TS包
int ts::demuxer::demux_ts_packet(const char* ptr, bool bInitInfo)
{
	if (!m_bDemuxerLiving)
	{
		return 0;
	...
	else
    {
        ...
        if( s.type!=0xff && 
			(((m_nDemuxID==0||m_nDemuxID==2)&&get_stream_type(s.type) == stream_type::h264_video) || //just  video
			((m_nDemuxID==1||m_nDemuxID==2)&&get_stream_type(s.type) == stream_type::aac_audio)) )	  //just  audio
        {
            // PES
            if(payload_unit_start_indicator)
            {
            ...
            if(s.frame_num)
            {
				...
				//获取音频信息
				if (get_stream_type(s.type) == stream_type::aac_audio&&(m_nSampleRate<0&&m_nChannels<0))//aac
				{
					unsigned long tnsSupportedSamplingRates[16] = //音频采样率标准，下表为写入标志
					{ 96000, 88200,64000,48000,44100,32000,24000,22050,16000,12000,11025,8000,7350,0,0,0 };

					int nSampRateId = (s.m_pCache[2]&0x3f)>>2;
					m_nSampleRate = tnsSupportedSamplingRates[nSampRateId] ;
					m_nChannels = (s.m_pCache[3])>>6;
					if (bInitInfo)
					{
						m_bDemuxerLiving = false;
						return 0;
					}
				}
            }
...
int ts::demuxer::demux_file(const char* name, bool bInitInfo)
{
    prefix.clear();
    char buf[192];
    int buf_len=0;
    ...
    for(u_int64_t pn=1;;pn++)
    {
        ...
        int n;
        if((n=demux_ts_packet(buf, bInitInfo)))
        {
